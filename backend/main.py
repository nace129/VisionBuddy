...
last_request_time = 0
last_frame_hash = ""
last_spoken_hash = ""  # ← track what we LAST SPOKE about
COOLDOWN_SECONDS = 3   # 3 seconds between auto scans
...
@app.get("/")
def root():
    return {"status": "VisionBuddy running 👁️"}
...
async def analyze(request: AnalyzeRequest):
    global last_request_time, last_frame_hash, last_spoken_hash
    now = time.time()

    # ── Rate limit auto scans to every 3 seconds ──────────
    ...
    frame_hash = hashlib.md5(request.image[:1000].encode()).hexdigest()

    # For auto scans — skip if same frame as last time
    if not request.force and not request.question:
    ...
    try:
        ...
        # Force analyze on manual tap
        if request.force and not response_text:
            response_text = analyze_image_nvidia(request.image)
            should_speak = True

        # ── Only speak if description is NEW ──────────────
        if response_text:
            desc_hash = hashlib.md5(response_text[:100].encode()).hexdigest()
            if desc_hash == last_spoken_hash and not request.force:
                should_speak = False
            else:
                last_spoken_hash = desc_hash
                should_speak = True

        # Force speak on manual tap always
        if request.force:
            should_speak = True
...
#     # Rate limit auto scans only
#     if not request.force and not request.question:
#         if now - last_request_time < COOLDOWN_SECONDS:
#             return {"description": "", "should_speak": False,
#                     "action_taken": "rate_limited", "mode_used": "none"}

#     # Frame change detection for auto scans
#     frame_hash = hashlib.md5(request.image[:500].encode()).hexdigest()
#     if not request.force and not request.question:
#         if frame_hash == last_frame_hash:
#             return {"description": "", "should_speak": False,
#                     "action_taken": "no_change", "mode_used": "none"}
...
#         # Force speak on manual tap even if empty
#         if request.force and not response_text:
#             response_text = analyze_image_nvidia(request.image)

#         return {
#             "description": response_text,
#             "should_speak": result.get("should_speak", True) if not request.force else True,
#             "action_taken": result.get("action_taken", "general"),
#             "mode_used": result.get("mode_used", "general"),
#             "scene_changed": result.get("scene_changed", True),
#         }
...
#     try:
#         result = nemotron_followup(request.question)
#         return {"answer": result.get("response", ""), "should_speak": True}
#     except Exception as e:
#         raise HTTPException(status_code=500, detail=str(e))
